...
- **Rest alert with the screen off** (`src/lib/restAlert.ts`, opt-in setting `timerLockScreen`): a locked phone freezes the page's timers but keeps media playing, so the rest plays as a generated WAV (near-silence, then the beep) and the beep comes from the audio itself. Tested on the owner's Android phone: it beeps while locked and Spotify keeps playing. Because the track is near-silent, Chrome doesn't treat it as media, so there's no lock-screen player; a "Resting until 21:43" notification is shown while the app is hidden instead. Taps on it are handled by `public/sw-notifications.js`, which is imported into the generated service worker.
